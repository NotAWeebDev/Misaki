...
class Purge extends Command {
  ...
  async run(message, args, level) { // eslint-disable-line no-unused-vars
    if (args[0].isNumber() == true && args[1] == null) {
      ...
      message.channel.bulkDelete(todelete).catch(error => console.log(error.stack));
      message.channel.send(this.client.responses.purgeMessages.random().replaceAll("{{amount}}", todelete.size)).then((msg) => msg.delete(10000));
    }
  }
}
