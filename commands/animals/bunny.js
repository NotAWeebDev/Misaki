...
class Bunny extends Social {
  ...
  async run(message, args, level) { // eslint-disable-line no-unused-vars
    try {
      ...
      await msg.edit({embed:{ "title": "Click here if the image failed to load.", "url": body.media.gif, "color":message.guild.member(this.client.user.id).highestRole.color || 5198940, "image": {"url": body.media.gif}}});
    } catch (e) {
      console.log(e);
    }
  }
